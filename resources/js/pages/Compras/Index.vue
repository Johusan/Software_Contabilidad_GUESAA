                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4 border-zinc-100 dark:border-zinc-850">
                            ...
                            <div class="p-4 rounded-lg bg-zinc-50 dark:bg-zinc-800/20 border border-zinc-150 dark:border-zinc-800/80 text-xs">
                                ...
                                <div class="font-mono text-[10px] space-y-1 bg-white dark:bg-zinc-950 p-2 rounded border">
                                    <div class="flex justify-between"><span>601 Mercaderías</span> <span class="text-emerald-600 font-semibold">DEBE: {{ formatCurrency(computedSubtotal) }}</span></div>
                                    <div class="flex justify-between"><span>401 Gobierno central (IGV)</span> <span class="text-emerald-600 font-semibold">DEBE: {{ formatCurrency(computedIgv) }}</span></div>
                                    <div class="flex justify-between pl-4"><span>421 Facturas por pagar</span> <span class="text-blue-600 font-semibold">HABER: {{ formatCurrency(computedTotal) }}</span></div>
                                    <hr class="my-1 border-zinc-100 dark:border-zinc-800" />
                                    <div class="flex justify-between"><span>201 Mercaderías</span> <span class="text-emerald-600 font-semibold">DEBE: {{ formatCurrency(computedSubtotal) }}</span></div>
                                    <div class="flex justify-between pl-4"><span>611 Variación de existencias</span> <span class="text-blue-600 font-semibold">HABER: {{ formatCurrency(computedSubtotal) }}</span></div>
                                </div>
                            </div>
